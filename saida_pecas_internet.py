...
# --- LEITURA DA ABA DE CLIENTES ---
@st.cache_data(ttl=300)  # Atualiza a lista de clientes a cada 5 minutos
...
def obter_proximo_numero_master():
    try:
        # Força o ttl=0 para sempre ler o dado mais recente da nuvem e evitar duplicidade
...
# Inicializa o número estável na sessão do usuário atual
...
if opcao_menu == "📝 Gerar Aceite":
    ...
    # Recalcula o número Master em tempo de execução para evitar colisões entre usuários
    st.session_state.numero_documento = obter_proximo_numero_master()
    ...
    if btn_gravar:
        ...
        try:
            # Puxa o histórico atual da nuvem, adiciona a nova linha e atualiza a planilha
            df_hist = conn.read(worksheet="historico_aceites", ttl=0)
            ...
            # Reseta o número estável da sessão para recalcular no próximo carregamento
            if "numero_documento" in st.session_state:
                del st.session_state.numero_documento
        except Exception as e:
            st.error(f"Erro ao salvar na planilha: {e}. Verifique se ela está configurada como Editor.")

    if campos_validos:
        num_exib_pdf = f"Controle Master: #{num_controle_salvar}" if tipo_contrato == "Master" else f"Reparo: {num_controle_salvar}"
        pdf_bytes = gerar_pdf_bytes(
            cliente_selecionado, endereco_selecionado, codigo_selecionado, 
            tipo_contrato, num_exib_pdf, nome_tecnico, nome_peca, codigo_rastreio)
        st.write(" ")
